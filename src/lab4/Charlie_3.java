...
public class Charlie_3 {
	...
	public void moveTillSense(double d) {
		...
		while (sample_sonic[0] > d && this.distToGoal(this.x, this.y) > .20 && !this.stop && !this.frontBump()) {
...
//		System.out.println("Turned in place");
//		this.buttonWait();
	}
...
//	}
	...
	public boolean inMLineRange() {
		if (this.y < (2.25 * this.x - 2.15) && this.y > (2.25 * this.x - 2.35) && this.y < 1.8) {
			return true;
		}
		return false;
	}
...
//		}
	...
	public boolean returnedToHP() {
		return this.inRange(this.hitX, this.hitY, .10);
	}
	...
	public void bug2() {
		...
		while (!Button.ENTER.isDown() && this.distToGoal(this.x, this.y) > .20 && !this.stop) {
...
//		System.out.println("time to wait: " + (System.nanoTime() - startTime > 3 * Math.pow(10, 9)));
		while (!Button.ENTER.isDown()
				...
				&& !(returnedToHP() && (System.nanoTime() - startTime > 3 * Math.pow(10, 9))) && !this.stop) {
